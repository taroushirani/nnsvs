...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for s in ${datasets[@]};
    do
	nnsvs-prepare-features --config-dir hydra/prepare_features --config-path config.yaml \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for s in ${testsets[@]}; do
        for typ in timelag duration acoustic; do
            ...
            xrun nnsvs-generate --config-dir hydra/generate --config-path config.yaml \
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    ...
    for s in ${testsets[@]}; do
        for input in label_phone_score label_phone_align; do
            ...
            xrun nnsvs-synthesis --config-dir hydra/synthesis --config-path config.yaml \
		 question_path=conf/jp_qst001_nnsvs.hed \
